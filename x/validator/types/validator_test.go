...
func TestValidatorTestEquivalent(t *testing.T) {
	...
	bz1, err := val1.Marshal()
	require.NoError(t, err)
	bz2, err := val2.Marshal()
	require.NoError(t, err)
...
func TestABCIValidatorUpdate(t *testing.T) {
	...
	require.Equal(t, validator.Stake, abciVal.Power)
}
...
func TestValidatorsSortTendermint(t *testing.T) {
	...
	expectedVals, err := testvalidator.ToTmValidators(valz)
	require.NoError(t, err)
	...
	sort.SliceStable(valz, func(i, j int) bool {
		return types.ValidatorsByVotingPower(valz).Less(i, j)
	})
	actualVals, err := testvalidator.ToTmValidators(valz)
	require.NoError(t, err)
...
func TestValidatorToTm(t *testing.T) {
	...
	vs, err := testvalidator.ToTmValidators(vals)
	require.NoError(t, err)
	require.Equal(t, expected, vs)
}

func TestBondStatus(t *testing.T) {
	require.False(t, types.BondStatus_Unbonded == types.BondStatus_Bonded)
	require.False(t, types.BondStatus_Unbonded == types.BondStatus_Unbonding)
	require.False(t, types.BondStatus_Bonded == types.BondStatus_Unbonding)
	require.Equal(t, types.BondStatus(4).String(), "4")
	require.Equal(t, types.BondStatusUnspecified, types.BondStatus_Unspecified.String())
	require.Equal(t, types.BondStatusUnbonded, types.BondStatus_Unbonded.String())
	require.Equal(t, types.BondStatusBonded, types.BondStatus_Bonded.String())
	require.Equal(t, types.BondStatusUnbonding, types.BondStatus_Unbonding.String())
}
...
func newValidator(t *testing.T, operator sdk.ValAddress, pubKey cryptotypes.PubKey) types.Validator {
	v, err := types.NewValidator(
		operator,
		sdk.AccAddress(operator),
		pubKey,
