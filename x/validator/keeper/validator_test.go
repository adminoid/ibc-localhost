...
func bootstrapValidatorTest(t testing.TB, power int64, numAddrs int) (*app.DSC, sdk.Context, []sdk.AccAddress, []sdk.ValAddress) {
	_, app, ctx := createTestInput(&testing.T{})

	coins := sdk.NewCoins(sdk.NewInt64Coin(app.ValidatorKeeper.BaseDenom(ctx), 100))
	addrDels, addrVals := generateAddresses(app, ctx, numAddrs, coins)

	amt := keeper.TokensFromConsensusPower(power)
	totalSupply := sdk.NewCoins(sdk.NewCoin(app.ValidatorKeeper.BaseDenom(ctx), amt.MulRaw(int64(len(addrDels)))))

	notBondedPool := app.ValidatorKeeper.GetNotBondedPool(ctx)
	...
	delegations := app.ValidatorKeeper.GetAllDelegations(ctx)
	...
	keeper.EndBlocker(ctx, app.ValidatorKeeper, abci.RequestEndBlock{})

	return app, ctx, addrDels, addrVals
}

func TestSetGetValidator(t *testing.T) {
...
func TestSetGetValidatorByConsAddr(t *testing.T) {
	...
	// Set/Get ValidatorByConsAddr
	err = dsc.ValidatorKeeper.SetValidatorByConsAddr(ctx, val)
	require.NoError(t, err)
	consAdr, err := val.GetConsAddr()
	require.NoError(t, err)
...
//			vs[i] = testvalidator.NewValidator(t, sdk.ValAddress(addrs[i]), pks[i])
...
//		validator := testvalidator.NewValidator(t, valAddr, valPubKey)
...
//		powers := []int64{9, 8, 7}
//		for i, power := range powers {
//			validators[i] = testvalidator.NewValidator(t, addrVals[i], PKs[i])
...
//			validators[i] = testvalidator.NewValidator(t, sdk.ValAddress(addrs[i]), PKs[i])
...
//		// initialize some validators into the state
//		var validators [3]types.Validator
//		validators[0] = testvalidator.NewValidator(t, sdk.ValAddress(PKs[0].Address().Bytes()), PKs[0])
//		validators[1] = testvalidator.NewValidator(t, sdk.ValAddress(addrs[1]), PKs[1])
//		validators[2] = testvalidator.NewValidator(t, sdk.ValAddress(addrs[2]), PKs[2])
//
...
//			valAddr := sdk.ValAddress(valPubKey.Address().Bytes())
//
//			validators[i] = testvalidator.NewValidator(t, valAddr, valPubKey)
//			tokens := app.StakingKeeper.TokensFromConsensusPower(ctx, power)
//			validators[i], _ = validators[i].AddTokensFromDel(tokens)
//		}
//
...
//
//		val1 := testvalidator.NewValidator(t, addrVals[0], PKs[0])
//		val2 := testvalidator.NewValidator(t, addrVals[1], PKs[1])
//
