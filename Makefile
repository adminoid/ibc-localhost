...
# todo - here `netgo` set, that appears in make install
build_tags = netgo
ifeq ($(LEDGER_ENABLED),true)
  ifeq ($(OS),Windows_NT)
    ...
    ifeq ($(UNAME_S),OpenBSD)
      $(warning OpenBSD detected, disabling ledger support (https://github.com/adminoid/cosmos-sdk/issues/1988))
    else
...
ldflags = -X github.com/adminoid/cosmos-sdk/version.Name=Decimal \
          -X github.com/adminoid/cosmos-sdk/version.AppName=$(DECIMAL_BINARY) \
          -X github.com/adminoid/cosmos-sdk/version.Version=$(VERSION) \
          -X github.com/adminoid/cosmos-sdk/version.Commit=$(COMMIT) \
          -X "github.com/adminoid/cosmos-sdk/version.BuildTags=$(build_tags_comma_sep)" \
          -X github.com/cometbft/cometbft/version.TMCoreSemVer=$(TMVERSION)

# DB backend selection
ifeq (cleveldb,$(findstring cleveldb,$(COSMOS_BUILD_OPTIONS)))
  ldflags += -X github.com/adminoid/cosmos-sdk/types.DBBackend=cleveldb
endif
ifeq (badgerdb,$(findstring badgerdb,$(COSMOS_BUILD_OPTIONS)))
  ldflags += -X github.com/adminoid/cosmos-sdk/types.DBBackend=badgerdb
endif
# handle rocksdb
ifeq (rocksdb,$(findstring rocksdb,$(COSMOS_BUILD_OPTIONS)))
  CGO_ENABLED=1
  BUILD_TAGS += rocksdb
  ldflags += -X github.com/adminoid/cosmos-sdk/types.DBBackend=rocksdb
endif
# handle boltdb
ifeq (boltdb,$(findstring boltdb,$(COSMOS_BUILD_OPTIONS)))
  BUILD_TAGS += boltdb
  ldflags += -X github.com/adminoid/cosmos-sdk/types.DBBackend=boltdb
endif
...
build-docker:
	...
	$(DOCKER) create --name dsc -t -i ${DOCKER_IMAGE}:latest dsc
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp dsc:/usr/bin/dscd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/bitbucket.org/decimalteam/go-smart-node/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(DECIMAL_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(DECIMAL_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(DECIMAL_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(DECIMAL_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local bitbucket.org/decimalteam/go-smart-node
.PHONY: format
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(DECIMAL_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\dsc\Z dscd/node "./dscd testnet --v 4 -o /dsc --keyring-backend=test --ip-addresses dscdnode0,dscdnode1,dscdnode2,dscdnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(DECIMAL_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/dsc:Z dscd/node "./dscd testnet --v 4 -o /dsc --keyring-backend=test --ip-addresses dscdnode0,dscdnode1,dscdnode2,dscdnode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\dscd:dsc\Z dscd/node "./dscd unsafe-reset-all --home=/dsc"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\dscd:dsc\Z dscd/node "./dscd unsafe-reset-all --home=/dsc"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\dscd:dsc\Z dscd/node "./dscd unsafe-reset-all --home=/dsc"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\dscd:dsc\Z dscd/node "./dscd unsafe-reset-all --home=/dsc"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/dscd:/dsc:Z dscd/node "./dscd unsafe-reset-all --home=/dsc"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/dscd:/dsc:Z dscd/node "./dscd unsafe-reset-all --home=/dsc"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/dscd:/dsc:Z dscd/node "./dscd unsafe-reset-all --home=/dsc"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/dscd:/dsc:Z dscd/node "./dscd unsafe-reset-all --home=/dsc"
endif
...
.PHONY: build-docker-local-dsc localnet-start localnet-stop
